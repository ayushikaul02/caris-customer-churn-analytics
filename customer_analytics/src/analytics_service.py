...
class CustomerAnalyticsService:
    ...
    def analyze_churn(self, df: pd.DataFrame) -> dict:
        ...
        if 'customer_segment' in df.columns:
            ...
            segment_churn = df.groupby('customer_segment')['status'].apply(
                lambda x: round((x == 'churned').sum() / len(x) if len(x) > 0 else 0, 4)
            ).to_dict()
            churn_analysis['churn_by_segment'] = segment_churn
        
        if 'segment_label' in df.columns:
            segment_label_churn = df.groupby('segment_label')['status'].apply(
                lambda x: round((x == 'churned').sum() / len(x) if len(x) > 0 else 0, 4)
            ).to_dict()
    ...
    def calculate_risk_score(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if risk_factors:
            df['churn_risk_score'] = sum(
                df[factor] * weight for factor, weight in risk_factors
            )
    ...
    def calculate_clv(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'total_spent' in df.columns and 'tenure_months' in df.columns:
            ...
            df['avg_monthly_spend'] = df.get('monthly_charge', 50)
        
        # Estimate lifetime months
        if 'tenure_months' in df.columns:
            df['lifetime_months'] = df['tenure_months'] * 1.5
        else:
            df['lifetime_months'] = 36
        
        # Profit margin (60%)
        profit_margin = 0.6
        
        # Calculate CLV
        df['clv'] = df['avg_monthly_spend'] * df['lifetime_months'] * profit_margin
        
        # Categorize CLV
        df['clv_category'] = pd.qcut(
            df['clv'],
            q=4,
        ...
        # CLV percentile
        df['clv_percentile'] = df['clv'].rank(pct=True)
        
        logger.info(f"✅ CLV calculated for {len(df)} customers")
        return df

    # ================ NEW: REVENUE IMPACT ANALYSIS ================

    def calculate_revenue_impact(self, df: pd.DataFrame) -> dict:
        """Calculate revenue impact analysis"""
        logger.info("Calculating revenue impact analysis...")
        
        impact = {}
        
        if 'total_spent' in df.columns and 'status' in df.columns:
            # High-risk customers
            ...
            revenue_at_risk = len(high_risk_df) * avg_revenue
            
            # Churned customers
            churned_df = df[df['status'] == 'churned']
            lost_revenue = churned_df['total_spent'].sum()
            ...
            impact = {
                ...
                'retention_roi': round(potential_savings / (len(high_risk_df) * 100) if len(high_risk_df) > 0 else 0, 2)
            }
        
        logger.info("✅ Revenue impact analysis completed")
        return impact